...
"""
these codes are copied from https://github.com/paulguerrero/pcpnet/blob/master/eval_pcpnet.py
"""
def parse_arguments():
    ...
    # parser.add_argument('--modeldir', type=str, default='./checkpoints', help='model folder')
...
def eval_pcpnet(opt):
    ...
    for model_name in opt.models:
        ...
        if opt.batchSize == 0:
            model_batchSize = trainopt.train.batchsize
        else:
            model_batchSize =trainopt.train.batchsize
        ...
        pred_dim = 0
        output_pred_ind = []
        for o in trainopt.pcpnet.outputs:
        ...
        dataset = PointcloudPatchDataset(
            ...
            patch_radius=trainopt.train.patch_radius,
            points_per_patch=trainopt.pcpnet.points_per_patch,
            patch_features=[],
            seed=opt.seed,
            use_pca=trainopt.train.use_pca,
            center=trainopt.train.patch_center,
            point_tuple=trainopt.pcpnet.point_tuple,
        ...
        if len(trainopt.train.patch_radius) == 1:
            regressor = SPCPNet(
                num_points=trainopt.pcpnet.points_per_patch,
                output_dim=pred_dim,
                use_point_stn=trainopt.pcpnet.use_point_stn,
                use_feat_stn=trainopt.pcpnet.use_feat_stn,
                sym_op=trainopt.pcpnet.sym_op,
                point_tuple=trainopt.pcpnet.point_tuple)
        else:
            regressor = MPCPNet(
                num_scales=len(trainopt.pcpnet.patch_radius),
                num_points=trainopt.pcpnet.points_per_patch,
                output_dim=pred_dim,
                use_point_stn=trainopt.pcpnet.use_point_stn,
                use_feat_stn=trainopt.pcpnet.use_feat_stn,
        ...
        trained_best = "checkpoints/model_best.pth.tar"
        # model.load_state_dict(torch.load(trained_best)['state_dict'])
        regressor.load_state_dict(torch.load(trained_best)['state_dict'])
        ...
        for batchind, data in batch_enum:
            ...
            while batch_offset < pred.size(0):
                ...
                if shape_patches_remaining <= batch_patches_remaining:
                    ...
                    prop_saved = [False]*len(trainopt.pcpnet.outputs)

                    # save normals
                    oi = [i for i, o in enumerate(trainopt.pcpnet.outputs) if o in ['unoriented_normals', 'oriented_normals']]
                    if len(oi) > 1:
                    ...
                    # save curvatures
                    oi1 = [i for i, o in enumerate(trainopt.pcpnet.outputs) if o == 'max_curvature']
                    oi2 = [i for i, o in enumerate(trainopt.pcpnet.outputs) if o == 'min_curvature']
